...
DRIVER_ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
DRIVER_IN_DOCKER=false


# If inside docker container
if [ -f /.dockerenv ]; then
    DRIVER_IN_DOCKER=true
    DRIVER_ROOT_DIR="/workspace"
fi

export DRIVER_ROOT_DIR="${DRIVER_ROOT_DIR}"
export DRIVER_IN_DOCKER="${DRIVER_IN_DOCKER}"
export DRIVER_CACHE_DIR="${DRIVER_ROOT_DIR}/.cache"
export DRIVER_SYSROOT_DIR="/opt/driver/sysroot"
...
function warning_green() {
  (echo >&2 -e "${GREEN}[WARNING] $*${NO_COLOR}")
}
